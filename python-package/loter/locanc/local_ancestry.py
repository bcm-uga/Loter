...
def locanc_g_knn(l_h, g_adm, penalty=40, small_penalty=0):
    A_in = np.ascontiguousarray(np.vstack(l_h))
    S_adm, H = learn_S_join({"G": g_adm}, A_in, penalty, small_penalty)
...
def loter_multiple_pops(l_H, h_adm, lambd):
    res_loter, _= locanc_h_knn([h.astype(np.uint8) for h in l_H],
                               h_adm.astype(np.uint8), lambd, 20)
    return res_loter

def boostrap_loter_multiple_pops(l_H, h_adm, lambd, counts, nbrun=20):
...
def loter_local_ancestry(l_H, h_adm, range_lambda=np.arange(1.5, 5.5, 0.5), rate_vote=0.5, nb_bagging=20):
    input_loter = (l_H, h_adm)
    n, m = h_adm.shape
    counts = np.zeros((len(l_H), n, m))
    for l in range_lambda:
        res_boostrap = boostrap_loter_multiple_pops(*input_loter, lambd=l, counts=counts, nbrun=nb_bagging)
...
def find_lambda(s_in, threshold = 0.90, min_lambda = 1, max_lambda = 500):
    n, m = s_in.shape
    if max_lambda - min_lambda <= 1:
        return locanc_g_knn([np.zeros((1,m)), np.ones((1,m))],
                            s_in, min_lambda, min_lambda)
    else:
...
def loter_smooth(l_H, h_adm, range_lambda=np.arange(1.5, 5.5, 0.5), threshold=0.90, rate_vote=0.5, nb_bagging=20):
    res_impute, res_raw = loter_local_ancestry(l_H, h_adm, range_lambda, rate_vote, nb_bagging)
    ...
    for i in range(len(res_impute)):
        ...
        r, _, _ = find_lambda(arr_input, threshold=threshold)
        result_hap.append(r)
        result[i] = r[::2] + r[1::2]
